...
Feature: Circuit breaker can interact with a Redis server
  ...
  Scenario: The circuit breaker fails after all the tries when using the Redis persistence adapter
    Given A circuit breaker instance using Redis persistence adapter with "3" retries and a reset timeout of "5" seconds
  ...
  Scenario: The circuit breaker fails after all the tries when using the Redis persistence adapter
    Given A circuit breaker instance using Redis persistence adapter with "3" retries and a reset timeout of "15" seconds
    And it is wrapping a function that throws an exception
    And the circuit breaker will be in a "closed" state with "0" failures
    When the function is executed
    Then the circuit breaker will throw the exception from the function
  ...
  Scenario: The circuit breaker succeed after some failure when using the Redis persistence adapter
    Given A circuit breaker instance using Redis persistence adapter with "3" retries and a reset timeout of "5" seconds
  ...
  Scenario: The circuit breaker succeed at the first attempt when using the Redis persistence adapter
    Given A circuit breaker instance using Redis persistence adapter with "3" retries and a reset timeout of "5" seconds
